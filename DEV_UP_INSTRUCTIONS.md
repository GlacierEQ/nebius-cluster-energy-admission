...
**Repository:** `GlacierEQ/nebius-cluster-energy-admission`  
**Company lens (independent):** Nebius (`nebius`)  
**Innovation:** Cluster Energy Admission  
**Scaffold batch:** 2026-08-10T0924Z

## Mission

Implement a **real, testable** central mechanism that addresses the bottleneck below. Do **not** claim Nebius affiliation, proprietary access, or production deployment.
...
Admit jobs only under measured energy/power envelopes with explicit reject receipts.

## Hard rules (fail closed)

1. **No affiliation theater** — never state or imply Nebius employment, endorsement, or proprietary systems access.
...
File: `src/cluster_energy_admission.py`

- Expand `ClusterEnergyAdmission` into a complete, self-contained implementation.
- Public API must stay stable enough that tests in `tests/test_cluster_energy_admission.py` can be upgraded (not gutted).
- Include at least:
...
### 3. Strengthen tests
Files: `tests/test_cluster_energy_admission.py`, `tests/test_adversarial.py`
...
- Cloud deploy, customer pilots, proprietary Nebius APIs
